/*
 * based on https://github.com/Fang520/m4a_parse
 */
...
void iterate_samples_from_stsc(sample_cb_t sample_cb)
{
    int chunk_index = 0;
    int sample_index = 0;
    int i, j, k;
    ...
    for (i=0; i<stsc_num; i++) // stsc
    {
        ...
        for (j=0; j<n_chunk; j++) // chunk
        {
            ...
            for (k=0; k<n_sample; k++) // sample
            {
                ...
                sample_cb(chunk_offset + sample_size_total, size);
